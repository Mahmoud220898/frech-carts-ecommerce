...
import onlinePayment from "../../api/Payment";
import cashPayment from "../../api/Payment";
...
export default function Model({ id, clear }) {
  ...
  const { mutate, status, data } = useMutationCart(onlinePayment);
  const {
    mutate: mutatecash,
    status: cashstuts,
    data: cashdata,
  } = useMutationCart(cashPayment);
